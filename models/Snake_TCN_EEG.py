...
def run_training(seed, mlp_dim, n_mlp_layers, input_dim, train_loader, val_loader,
    ...
    if checkpoint_dir is None:
        checkpoint_dir = os.path.join(RESULTS_DIR, 'Snake_TCN_checkpoints')
    ...
    pd.DataFrame(metrics).to_csv(os.path.join(metrics_dir, f"Snake_TCN_d{mlp_dim}_l{n_mlp_layers}_seed{seed}_metrics.csv"), index=False)
