# 2024 12 31: With Absolute folder path -> can run in batch
# 2024 12 31: Config-1 for baseline
...
def saving(exp_name, exp_index, train_len, iteration_list, train_loss_list, test_loss_list, model, simulation_result, ground_truth):
	...
	# join(dirname(abspath(__file__)), exp_name)

	if not os.path.exists(join(dirname(abspath(__file__)), exp_name)):
		os.makedirs(join(dirname(abspath(__file__)), exp_name))

	# Save the train loss list
	with open(join(dirname(abspath(__file__)), exp_name + "/" + file_name + "_TRAINING_LOST" + ".txt"), "wb") as fp:
		pickle.dump(train_loss_list, fp)

	# Save the test loss list
	with open(join(dirname(abspath(__file__)), exp_name + "/" + file_name + "_TESTING_LOST" + ".txt"), "wb") as fp:
		pickle.dump(test_loss_list, fp)

	# Save the simulation result
	with open(join(dirname(abspath(__file__)), exp_name + "/" + file_name + "_SIMULATION_RESULT" + ".txt"), "wb") as fp:
		pickle.dump(saved_simulation_truth, fp)

	# Save the model parameters
	# torch.save(model.state_dict(), exp_name + "/" +  file_name + "_torch_model.pth")
	torch.save(model.state_dict(), join(dirname(abspath(__file__)), exp_name + "/" +  file_name + "_torch_model.pth"))
...
def plotting_data(exp_name, exp_index, file_name, iteration_list, train_loss_list, test_loss_list):
	...
	ax.set(xlabel='Epoch', 
		   title=exp_name)
	fig.savefig(join(dirname(abspath(__file__)), exp_name + "/" + file_name + "_" + "loss" + "_"+ datetime.now().strftime("NO%Y%m%d%H%M%S") + ".pdf") , format='pdf')
	plt.clf()
...
def plotting_simulation(exp_name, exp_index, file_name, train_len, simulation_result, ground_truth):
	...
	plt.savefig(join(dirname(abspath(__file__)), exp_name + "/" + file_name + "_" + "simulation" + "_"+ datetime.now().strftime("NO%Y%m%d%H%M%S") + ".pdf") , format='pdf')
	return
...
def main():
	...
	exp_name = "QLSTM_CONFIG_1_TS_MODEL_NARMA_10_1"
	exp_index = 1
